...
```{r}
commonAnimals = intersect(rownames(df_mammals),rownames(df_anim))
commonAnimals
```
...
```{r}
setdiff(rownames(df_mammals),rownames(df_anim))
#Son 4 animales.
```
...
```{r}
#La diferencia entre estos animales se encuentra principalmente en el cerebro y el cuerpo.
```
...
qqPlot(log(df_anim$brain))
#Podemos decir que ambas representaciones siguen una distribución Gaussiana,lo que sí es cierto es que algunos puntos se salen de las líneas discontinuas y seguramente sean los animales que no aparecen en ambos conjuntos.
```
...
#Tengo instaladas 236 librerías.
```
...
summary(df_cabb)
summary(df_chi)
#Vemos que en el conjunto Chile hay valores perdidos en las variables edad,ingresos,votos y estatus.
```
...
```{r}
qqPlot(df_cabb$HeadWt)
qqPlot(df_cabb$VitC)
#Podemos ver que ambas variables siguen en gran parte una distribución Gaussiana.
```
...
```{r}
hist(df_cabb$VitC[df_cabb$Cult == "c39"],main="Diferencia con variable c39")
hist(df_cabb$VitC[df_cabb$Cult == "c52"],main="Diferencia con variable c52")
```
...
```{r}
plot(df_cabb$HeadWt,df_cabb$VitC)
plot(df_cabb$HeadWt, df_cabb$VitC, pch=c(6,16)[df_cabb$Cult])
legend(x="topright",legend=c("Cult = c39","Cult = c52"), pch=c(6,16))
Model3 = lm(HeadWt ~ VitC, data = cabbages, subset = which(Cult == "c39"))
Model4 = lm(HeadWt ~ VitC, data = cabbages, subset = which(Cult == "c52"))
abline(Model3,lty = 2)
abline(Model4)
```
